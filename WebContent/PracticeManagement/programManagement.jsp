					...
					<div class="pmd-card pmd-z-depth pmd-card-custom-view">
						...
						<div class="table-responsive">
								...
								<tbody>
									<c:forEach items="${selectProjects }" var="selectProject">
										<tr>
											...
											<td>${selectProject.major }</td>
											<td>${selectProject.introduction }</td>
											<td>${selectProject.companyTeacher }</td>
											<td>${selectProject.releaseDate }</td>
											<td>${selectProject.auditDate!=null?"已审核":"未审核" }</td>
											<c:if test="${selectProjectsRole==1&&selectProject.auditDate==null }">
												<td><a
													href="UpdatePracticeServlet?no=${selectProject.no }">修改</a></td>
											</c:if>
										</tr>
									</c:forEach>
								</tbody>
							...
							<div class="button-group col-md-12">
								<button type="button" class="btn pmd-ripple-effect btn-primary">
									确定</button>
							</div>
						</div>
					</div>
